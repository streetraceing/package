...
const commands = new Set(['zip', 'shift', 'diff', 'apply', 'inspect', 'check', 'list', 'init', 'help', 'version']);
...
function takeValue(argv: string[], index: number, flag: string): { value: string; next: number } {
  ...
  if (!value || value.startsWith('--')) throw new PackageError(`${flag} requires a value.`, 'CLI_ARGUMENT');
...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--help' || arg === '-h') rawPositionals.unshift('help');
    else if (arg === '--version' || arg === '-v') rawPositionals.unshift('version');
    ...
    else if (arg === '--follow-symlinks') parsed.configOverrides.followSymlinks = true;
    else if (arg === '--no-follow-symlinks') parsed.configOverrides.followSymlinks = false;
      ...
      if (result.value !== 'git' && result.value !== 'walk') throw new PackageError('--strategy must be git or walk.', 'CLI_ARGUMENT');
      parsed.configOverrides.strategy = result.value;
      index = result.next;
    } else if (arg.startsWith('--ignore')) {
      const result = takeValue(argv, index, '--ignore');
      parsed.configOverrides.ignore = [...(parsed.configOverrides.ignore ?? []), result.value];
      index = result.next;
    } else if (arg.startsWith('--include')) {
      const result = takeValue(argv, index, '--include');
      parsed.configOverrides.include = [...(parsed.configOverrides.include ?? []), result.value];
      index = result.next;
      ...
      if (result.value !== 'abort' && result.value !== 'overwrite' && result.value !== 'skip') {
        throw new PackageError('--conflict must be abort, overwrite, or skip.', 'CLI_ARGUMENT');
      }
