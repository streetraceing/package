...
import { checkCommand, initCommand, inspectCommand, listCommand } from '../commands/meta.js';
...
async function readVersion(): Promise<string> {
  ...
  for (const candidate of candidates) {
    try {
      const value = JSON.parse(await readFile(candidate, 'utf8')) as { version?: string };
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  let projectCwd = cwd;
  if (args.command === 'zip' && args.positionals[0]) projectCwd = path.resolve(cwd, args.positionals[0]);
  const loaded = await loadConfig(projectCwd, args.configPath);
  const merged = { ...loaded.config, ...args.configOverrides };
  if (args.configOverrides.ignore) merged.ignore = [...loaded.config.ignore, ...args.configOverrides.ignore];
  if (args.configOverrides.include) merged.include = args.configOverrides.include;
  ...
  if (args.command === 'zip') {
    ...
    await createShiftArchive(requirePositional(args.positionals, 0, 'base archive'), config, {
      output: args.output,
      message: args.message,
      quiet: args.quiet,
    });
  } else if (args.command === 'diff') {
    const exitCode = await diffCommand(requirePositional(args.positionals, 0, 'archive path'), config.root, args.json);
    ...
    await inspectCommand(requirePositional(args.positionals, 0, 'archive path'), config.root, args.json);
  } else if (args.command === 'check') {
    await checkCommand(requirePositional(args.positionals, 0, 'archive path'), config.root);
