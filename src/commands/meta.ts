...
export async function inspectCommand(archivePath: string, cwd: string, json: boolean): Promise<void> {
  const pkg = await loadPackage(path.resolve(cwd, archivePath));
  const operations = pkg.shift?.instructions.filter((instruction) => instruction.type !== 'MESSAGE' && instruction.type !== 'BASE').length ?? 0;
  if (json) {
    console.log(JSON.stringify({ manifest: pkg.manifest, structuralOperations: operations }, null, 2));
    return;
  }
  ...
  if (pkg.manifest.baseRootHash) console.log(`Base: ${pkg.manifest.baseRootHash}`);
}

export async function checkCommand(archivePath: string, cwd: string): Promise<void> {
  const pkg = await loadPackage(path.resolve(cwd, archivePath));
  console.log(`OK ${path.resolve(cwd, archivePath)}`);
  console.log(`${pkg.manifest.files.length} payload files verified`);
  if (pkg.shift) console.log(`${pkg.shift.instructions.length} PackageShift instructions parsed`);
}

export async function listCommand(archivePath: string | undefined, config: PackageConfig, json: boolean): Promise<void> {
  let paths: string[];
  if (archivePath) {
...
export async function initCommand(cwd: string, force: boolean): Promise<void> {
  ...
  if (!force) {
    try {
      ...
      throw new PackageError(`${target} already exists. Pass --force to overwrite it.`, 'CONFIG_EXISTS');
    } catch (error) {
